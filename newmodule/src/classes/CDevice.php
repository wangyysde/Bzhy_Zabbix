...
class CDevice extends CNewApiService  {
    ...
    public function getBrand($options=[]){
        ...
        if (!is_null($options['ids'])) {
            ...
            $sqlParts['where']['id'] = dbConditionInt('brandinfo.id', $options['ids']);
	}
        
        if (!is_null($options['local_name'])) {
            zbx_value2array($options['local_name']);
            $sqlParts['where']['local_name'] = dbConditionString('brandinfo.local_name', $options['local_name']);
	}
               
        if (!is_null($options['english_name'])) {
            zbx_value2array($options['english_name']);
            $sqlParts['where']['english_name'] = dbConditionString('brandinfo.english_name', $options['english_name']);
	}
        ...
        if ($options['selectOS'] !== null) {
            foreach ($result as $deviceid => $line){
                if(isset($line['osid']) && trim($line['osid']) != NULL && $line['osid'] != 0){
                    if(!is_null($OSInfo = $this->getOS(['ids'=>$line['osid'],'output'=>$options['selectType']]))){

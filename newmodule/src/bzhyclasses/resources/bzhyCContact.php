...
class bzhyCContact extends bzhyObjectCommon{
    ...
    public function get($options = []) {
        ...
        if(!is_null($options['status'])){
            ...
            $sqlParts['where']['contact_status'] = dbConditionInt(bzhyCDB::getFieldIdByObject($this->ObjectName,'contact_status'), $options['status']);
        }
        if (!is_null($options['object_ids']) && !is_null($options['object_table'])) {
            $sqlParts['from']['contactobjectrelation'] = 'contactobjectrelation cr';
            zbx_value2array($options['object_ids']);
            $sqlParts['where'][] = dbConditionInt('cr.object_value', $options['object_ids']);
            $sqlParts['where']['object_contact'] = 'cr.contact_id = '.bzhyCDB::getFieldIdByObject($this->ObjectName,'id');
            $sqlParts['where']['object_table'] = 'cr.object_table =\''.$options['object_table'].'\'';
	}

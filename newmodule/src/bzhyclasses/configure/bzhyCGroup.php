 ...
 */
class bzhyCGroup extends bzhyObjectCommon {
    ...
    */
	...
	$sqlParts = [
            'select'	=> [$this->tableName => $this->tableAlias.'.groupid'],
	...
	if (!is_null($options['groupids'])) {
            ...
            $sqlParts['where']['groupid'] = dbConditionInt(bzhyCDB::getFieldIdByObject($this->ObjectName, 'groupid'), $options['groupids']);
	}
	...
	if (!is_null($options['hostids'])) {
            ...
            $sqlParts['where']['hgg'] = 'hg.groupid='.bzhyCDB::getFieldIdByObject($this->ObjectName, 'groupid');
	}
	...
	if (!is_null($options['maintenanceids'])) {
            ...
            $sqlParts['where']['hmh'] = bzhyCDB::getFieldIdByObject($this->ObjectName, 'groupid').'=mg.groupid';
	}
	...
	if ($options['monitored_hosts'] !== null) {
            $sub_sql_parts['from']['h'] = bzhyCDB::getFromByObject('host');
            $sub_sql_parts['where']['hg-h'] = 'hg.hostid='.bzhyCDB::getFieldIdByObject('host', 'hostid');
            $sub_sql_parts['where'][] = dbConditionInt(bzhyCDB::getFieldIdByObject('host', 'status'), [HOST_STATUS_MONITORED]);
	}
	elseif ($options['real_hosts'] !== null) {
            $sub_sql_parts['from']['h'] = bzhyCDB::getFromByObject('host');
            $sub_sql_parts['where']['hg-h'] = 'hg.hostid='.bzhyCDB::getFieldIdByObject('host', 'hostid');
            $sub_sql_parts['where'][] = dbConditionInt(bzhyCDB::getFieldIdByObject('host', 'status'), [HOST_STATUS_MONITORED, HOST_STATUS_NOT_MONITORED]);
	}
	elseif ($options['templated_hosts'] !== null) {
            $sub_sql_parts['from']['h'] = bzhyCDB::getFromByObject('host');
            $sub_sql_parts['where']['hg-h'] = 'hg.hostid='.bzhyCDB::getFieldIdByObject('host', 'hostid');
            $sub_sql_parts['where'][] = dbConditionInt(bzhyCDB::getFieldIdByObject('host', 'status'), [HOST_STATUS_TEMPLATE]);
	}
	elseif ($options['with_hosts_and_templates'] !== null) {
            $sub_sql_parts['from']['h'] = bzhyCDB::getFromByObject('host');
            $sub_sql_parts['where']['hg-h'] = 'hg.hostid='.bzhyCDB::getFieldIdByObject('host', 'hostid');
            $sub_sql_parts['where'][] = dbConditionInt(bzhyCDB::getFieldIdByObject('host', 'status'),
	...
	if (is_array($options['filter'])) {
            $this->dbFilter(bzhyCDB::getFromByObject($this->ObjectName), $options, $sqlParts);
	}
    ...
    */
	...
	foreach ($groups as $group) {
            $new_groupids = bzhyCDB::insert('groups', [$group]);
	...
	if ($groupids_with_rights) {
		...
		foreach ($parent_children[$right['id']] as $parent_child) {
            ...
            bzhyCDB::insert('rights', $rights_to_insert);
	}
    ...
    */
	...
	foreach ($groupids as $groupid) {
		...
		bzhyCBase::exception(ZBX_API_ERROR_PERMISSIONS,
                    _('No permissions to referred object or it does not exist!'),__FILE__,__LINE__,TRUE
		);
            }
            if ($delGroups[$groupid]['internal'] == ZBX_INTERNAL_GROUP) {
                bzhyCBase::exception(ZBX_API_ERROR_PARAMETERS,
                    _s('Host group "%1$s" is internal and can not be deleted.', $delGroups[$groupid]['name']),__FILE__,__LINE__,TRUE);
            }
	}
	...
	if (!empty($groupids)) {
            bzhyCDB::delete('sysmaps_elements', ['elementtype' => SYSMAP_ELEMENT_TYPE_HOST_GROUP, 'elementid' => $groupids]);
	}
	...
	if (!empty($actionids)) {
		...
		'where' => ['actionid' => $actionids]
            ...
            bzhyCDB::update('actions', $update);
	}

	// delete action conditions
	bzhycDB::delete('conditions', [
	...
	bzhyCDB::delete('operations', ['operationid' => $delOperationids]);

	bzhyCDB::delete('groups', ['groupid' => $groupids]);

	bzhyCDB::delete('profiles', [
    ...
    */
	...
	foreach ($groups as $group) {
		...
		bzhyCBase::exception(ZBX_API_ERROR_PARAMETERS, _('Incorrect arguments passed to function.'),__FILE__,__LINE__,TRUE);
            ...
            if ($missing_keys) {
		bzhyCBase::exception(ZBX_API_ERROR_PARAMETERS,
                    _s('Host group is missing parameters: %1$s', implode(', ', $missing_keys)),__FILE__,__LINE__,TRUE
		);
            }

            // Check if host group name is valid.
            if (!$this->validate($group['name'])) {
		bzhyCBase::exception(ZBX_API_ERROR_PARAMETERS,
                    _s('Incorrect value for field "%1$s".', 'name'),__FILE__,__LINE__,TRUE
		);
            }
	...
	if ($duplicate) {
            bzhyCBase::exception(ZBX_API_ERROR_PARAMETERS,
		_s('Duplicate "%1$s" value "%2$s" for host group.', 'name', $duplicate['name']),__FILE__,__LINE__,true
            );
	}

	// Check if host group already exists.
	$db_groups = bzhyCDB::select('groups', [
            'output' => ['name'],
	...
	if ($db_groups) {
            bzhyCBase::exception(ZBX_API_ERROR_PARAMETERS,
		_s('Host group "%1$s" already exists.', $db_groups[0]['name']),__FILE__,__LINE__,true
            );
	}
    }
    ...
    */
	...
	$host_group_name_validator = new CHostGroupNameValidator();
		...
		if (!$this->validate($group['name'])) {
                    bzhyCBase::exception(ZBX_API_ERROR_PARAMETERS,
			_s('Incorrect value for field "%1$s"', $group['name']),__FILE__,__LINE__,true 
                    );
		}
        ...
        if ($check_names) {
            ...
            $db_group_names = bzhyCDB::select('groups', [
		'output' => ['groupid', 'name'],
		...
		if (array_key_exists($group['name'], $db_group_names)
                    ...
                    bzhyCBase::exception(ZBX_API_ERROR_PARAMETERS, _s('Host group "%1$s" already exists.', $group['name']),__FILE__,__LINE__,TRUE);
		}
            }
	}
    ...
    */
	...
	foreach ($groupIds as $groupId) {
        ...
        bzhyCDB::insert('hosts_groups', $insert);
    ...
    */
	...
	bzhyCDB::replace('hosts_groups', $oldRecords, array_merge($replaceRecords, $newRecords));
	...
	*/
	...
	if (!$groupIdsToAdd && $groupIdsToRemove) {
		...
		bzhyCBase::exception(ZBX_API_ERROR_PARAMETERS, _('One of the objects is left without a host group.'),__FILE__,__LINE__,true);
            }
	}
    ...
    */
	...
	if ($options['selectGroupDiscovery'] !== null) {
            $groupDiscoveries = bzhyCDB::select('group_discovery', [
		...
		'preservekeys' => true
            ...
            $groupDiscoveries = bzhyCDB::unsetExtraFields($groupDiscoveries, ['groupid'],
    ...
    */
    protected function validate($name) {
        if (!is_string($name)) {
            bzhyCBase::exception(BZHY_ERROR_PARAMETERS,_('must be a string'),__FILE__,__LINE__);
            return false;
        }
	if ($name === '') {
            bzhyCBase::exception(BZHY_ERROR_PARAMETERS,_('cannot be empty'),__FILE__,__LINE__);
	}

	if ($name[0] === '/' || substr($name, -1) === '/' || strpos($name, '//') !== false
            || strpos($name, '*') !== false) {
            bzhyCBase::exception(BZHY_ERROR_PARAMETERS,_s('invalid group name "%1$s"', $name),__FILE__,__LINE__);
            return false;
	}
	return true;
    }

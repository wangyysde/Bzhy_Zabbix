...
class bzhyCDB {
    ...
    public static function insert($table, $values,$getids = true) {
       $resultIds = [];
        if (empty($values) || bzhy_empty($table)) {
            bzhyCBase::exception(BZHY_ERROR_PARAMETERS, _('Table name or values is empty.'),__FILE__,__LINE__,TRUE);
	}
        if(($Fields = self::getTableFields($table)) == FALSE){
            bzhyCBase::exception(self::DBEXECUTE_ERROR, _s('Get table fields error "%1$s".', $table),__FILE__,__LINE__,TRUE);
        }
        ...
        if($getids){
            ...
            if($ret){
                ...
                bzhyCBase::exception(self::DBEXECUTE_ERROR, _s('SQL statement execution has failed "%1$s".', $query));
            }
        }  
        foreach ($values as $key => $row) {
            ...
            if(self::checkFieldValues($row, $Fields['Fields']) == FALSE){
                bzhyCBase::exception(self::INPUT_ERROR, _s('Input Error'),__FILE__,__LINE__,TRUE);
            }
            $sql = 'INSERT INTO '.$table.' ('.implode(',', array_keys($row)).')'.
            ' VALUES ('.implode(',', array_values($row)).')';

            if (!DBexecute($sql)) {
                bzhyCBase::exception(self::DBEXECUTE_ERROR, _s('SQL statement execution has failed "%1$s".', $sql));
            }
            if($getids){
                $resultIds[] = $id++;
                $id = bcadd($id, 1, 0);
        ...
        foreach ($values as $field => $value){
            foreach ($Fields as $key =>$fieldData){
                if(strcasecmp($field, $fieldData["name"]) == 0 ){
                    if(zbx_empty($value) && ($fieldData['null'] == 0)){
                    ...
                    if(zbx_empty($value) && ($fieldData['null'] == 1)){
                        $value = $fieldData['default'];
                    }
    ...
    */
    public static function update($table, $data) {
        ...
        //		self::checkValueTypes($table, $row['values']);
            if (empty($row['values'])) {
                bzhyCBase::exception(self::DBEXECUTE_ERROR, _s('Cannot perform update statement on table "%1$s" without values.', $table),__FILE__,__LINE__,TRUE);
            }
            ...
            if (!isset($row['where']) || empty($row['where']) || !is_array($row['where'])) {
                bzhyCBase::exception(self::DBEXECUTE_ERROR, _s('Cannot perform update statement on table "%1$s" without where condition.', $table),__FILE__,__LINE__,TRUE);
            }
            ...
            foreach ($row['where'] as $field => $values) {
                if ((self::checkFieldValues($row['where'], $Fields['Fields']) == FALSE) || is_null($values)) {
                    bzhyCBase::exception(self::DBEXECUTE_ERROR, _s('Incorrect field "%1$s" name or value in where statement for table "%2$s".', $field, $table),__FILE__,__LINE__,TRUE);
                }
                $values = zbx_toArray($values);
            ...
            if (!DBexecute($sql)) {
                bzhyCBase::exception(self::DBEXECUTE_ERROR, _s('SQL statement execution has failed "%1$s".', $sql),__FILE__,__LINE__,TRUE);
            }
        }
    return true;
    }
    ...
    */
    ...
    public static function unSetUnusefulFields(&$values,$Fields = NULL,$table = NULL){
        ...
        if((zbx_empty($Fields) || !is_array($Fields)) && $table == NULL){ 
            bzhyCBase::exception(self::PARAMETER_ERROR, _s('Parameters error.')); 
        }
   ...
   public static function applyQueryFilterOptions($tableName, $tableAlias, array $options, array $sqlParts) {
        ...
        if (!zbx_empty($pkOption) && isset($options[$pkOption])) {
            ...
            $sqlParts['where'][] = dbConditionInt($fieldName, $options[$pkOption]);
        }
   ...
   public static function dbFilter($table, $options, &$sqlParts) {
        ...
        $table = $tableArray[0];
        $tableShort = isset($tableArray[1])?$tableArray[1]:NULL;
   ...
   public static function applyQueryOutputOptions($tableName, $tableAlias, array $options, array $sqlParts) {
	...
	elseif (is_array($options['output'])) {
            ...
            $sqlParts['select'][$Fields['id']] = $pkFieldId;
            foreach ($options['output'] as $field) {
		if (self::hasField($field, $tableName)) {
                    $sqlParts['select'][$field] = bzhy_empty($tableAlias)?$field:$tableAlias.".".$field; 
		}
            }
   ...
   public static function addQuerySelect($fieldId, array $sqlParts) {
        ...
        $fieldIdArray = explode('.', $fieldId);
        if(count($fieldIdArray) >1){
            $tableAlias = $fieldIdArray[0];
            $field = $fieldIdArray[1];
        }
        else{
            $tableAlias = NULL;
            $field = $fieldIdArray[0];
        }
	...
	if (!in_array($fieldId, $sqlParts['select']) && !in_array((bzhy_empty($tableAlias)?'*':$tableAlias.".*"), $sqlParts['select'])) {
            ...
            if ($field == '*') {
                if(!bzhy_empty($tableAlias)){
                    foreach ($sqlParts['select'] as $key => $selectFieldId) {
                        $selectFieldIdArray = explode('.', $selectFieldId);
                        if(count($selectFieldIdArray) > 1){
                            $selectTableAlias = $selectFieldIdArray[0];
   ...
   public static function applyQuerySortOptions($tableName, $tableAlias, array $options, array $sqlParts) {
	if (isset($options['sortfield']) && !bzhy_empty($options['sortfield'])) {
        $options['sortfield'] = is_array($options['sortfield'])?array_unique($options['sortfield'])
            :[$options['sortfield']];
    ...
    */
    ...
    public static function getTableFieldsByObject($object){
        ...
        $tableName = bzhyCBase::getTableByObject($object);
        if(bzhy_empty($tableName)){
            return NULL;
        }
    ...
    public static function dbSearch($table, $options, &$sql_parts) {
        ...
        if(count($tableArray) >1){
            $table = $tableArray[0];
            $tableShort = $tableArray[1];
        }
        else{
            $table = $tableShort = $tableArray[0];
        }
    ...
    * @return boolean
    */
    public static function hasField($fieldName, $tableName = null) {
    ...
    * @return bool
    */
    protected static function outputIsRequested($field, $output) {
	switch ($output) {
    ...
    public static function buildFieldId($tableAlias,$FieldName){
        if(bzhy_empty($FieldName)){
            $ret = "";
        }
        else{
            $ret = bzhy_empty($tableAlias)?$FieldName:$tableAlias.'.'.$FieldName;
        }
        
        return $ret;
    }
    
    public static function buildFromId($TableName,$TableAlias){
        if(bzhy_empty($TableName)){
            $ret = "";
        }
        else{
            $ret = bzhy_empty($TableAlias)?$TableName.' '.$TableName:$TableName.' '.$TableAlias;
        }

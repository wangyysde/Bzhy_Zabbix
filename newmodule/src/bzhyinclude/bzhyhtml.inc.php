...
function bzhyget_header_host_table($current_element, $hostid, $lld_ruleid = 0) {
    ...
    // get hosts
    $db_host = bzhyAPI::Host()->get($options);
    ...
    */
	...
	switch ($db_host['status']) {
        ...
        $host = new CSpan(new CLink($name, bzhyAPI::HostUri().'?form=update&hostid='.$db_host['hostid']));
	...
	$list->addItem([
            new CSpan(new CLink(_('All hosts'), bzhyAPI::HostUri().'?hostid='.$db_host['hostid'].url_param('groupid'))),
            '/',
            $host
	]);
	$list->addItem($status);

...
foreach ($data['hosts'] as $host) {
	...
	$description[] = new CLink(CHtml::encode($host['name']),
            bzhyCBase::getUriByObject('host').'?form=update&hostid='.$host['hostid'].url_param('groupid')
	);
	...
	if ($host['status'] == HOST_STATUS_MONITORED) {
		...
		$statusUrl = bzhyCBase::getUriByObject('host').'?hosts[]='.$host['hostid'].'&action=host.massdisable'.url_param('groupid');
	}
	else {
		$statusCaption = _('Disabled');
		$statusUrl = bzhyCBase::getUriByObject('host').'?hosts[]='.$host['hostid'].'&action=host.massenable'.url_param('groupid');

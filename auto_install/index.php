...
function str_replace_array($str,$replace_array){
...
if(isset($_REQUEST["action"])){
	...
	if(strtolower(trim($_REQUEST["action"])) === "ks"){    //KS信息处理
		  ...
		  if(!$result=mysql_db_query($DB["DATABASE"],$query,$DB_CONN)){
 			...
 			$MACRO_ARRAY["{DNS1}"] = $result_array[4];
 			$MACRO_ARRAY["{DNS2}"] = $result_array[5];
 			$MACRO_ARRAY["{GATEWAY}"] = $result_array[6];
		  ...
		  $MACRO_ARRAY["{KSURL}"] = $result_array[2];
			...
			if(!$fp=@fopen($filename,"r")){
 			...
 			for($i=0;$i<4;$i++){
 				if(($result_array=mysql_fetch_array($result))){
 					 $MACRO_ARRAY["{IP".($i+1)."}"] = $result_array[1];
 					 $MACRO_ARRAY["{MASK".($i+1)."}"] = $result_array[2];
 					 $MACRO_ARRAY["{EANBLED".($i+1)."}"] = $result_array[3];
 				}
 				else{
 					 $MACRO_ARRAY["{IP".($i+1)."}"] = "";
		  ...
		  while($result_array=mysql_fetch_array($result)){
		  	if($i == 0)
		  		$precommands="%pre\n";
		  	if($result_array[0] == 0){
		  		$precommands .= str_replace_array($result_array[1],$MACRO_ARRAY)."\n";
		  	}
		  	else{
		  ...
		  while($result_array=mysql_fetch_array($result)){
		  	if($result_array[0] == 0){
		  		$postcommands .= str_replace_array($result_array[1],$MACRO_ARRAY)."\n";
		  	}
		  	else{
		    	$command_array = split("/",$result_array[1]);
		    	$command_tpl = 	$command_array[0];
	...
	if(strtolower(trim($_REQUEST["action"])) === "commands"){     //处理命令请求
	    ...
	    if($command_type === "put_data"){         //向系统推送数据，即自动抓取网关的MAC地址、服务器型号、序列号等功能
		  ...
		  $result_array = mysql_fetch_array($result);
		  $MACRO_ARRAY["{DATATIME}"] = @date("YmdHis",time());
			$MACRO_ARRAY["{NETNAME}"] = $result_array[0];
			$MACRO_ARRAY["{HOSTNAME}"] = $result_array[1];
			$MACRO_ARRAY["{DNS1}"] = $result_array[2];
			$MACRO_ARRAY["{DNS2}"] = $result_array[3];
			$MACRO_ARRAY["{GATEWAY}"] = $result_array[4];
			$MACRO_ARRAY["{OSVER}"] = $osver;
 			$MACRO_ARRAY["{INSTALLMAC}"] =strtolower(str_replace(':',"-",trim($result_array[5])));
 			$MACRO_ARRAY["{HOSTID}"] = $hostid;
 			$MACRO_ARRAY["{INSTALLPASSWD}"] = $result_array[6];
			...
			$result_array = mysql_fetch_array($result);
			$MACRO_ARRAY["{OSHOMEURL}"] = $result_array[0];
		  $MACRO_ARRAY["{KSURL}"] = $result_array[2];
		  ...
		  $result_array=mysql_fetch_array($result);
		  $MACRO_ARRAY["{OSTYPE}"] = $result_array[0];
		  $MACRO_ARRAY["{OSIMGURL}"] = $MACRO_ARRAY["{OSHOMEURL}"].$result_array[1];
		  $MACRO_ARRAY["{INSTALL_SCRIPT}"] = $result_array[3].$INSTALL_SCRIPT;
		  ...
		  $MACRO_ARRAY["{BINDGATEWAYNICNO}"] = -1;
 			for($i=0;$i<4;$i++){
 				if(($result_array=mysql_fetch_array($result))){
 					 ...
 					 $MACRO_ARRAY["{MASK".($i+1)."}"] = $result_array[2];
 					 $MACRO_ARRAY["{EANBLED".($i+1)."}"] = $result_array[3];
 					 if((calc_net_address($result_array[1],$result_array[2]) == calc_net_address($MACRO_ARRAY["{GATEWAY}"],$result_array[2])) && ($result_array[3] == 0)){

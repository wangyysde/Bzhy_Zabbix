...
function calc_net_address($ip,$netmask){    //根据输入的IP地址和掩码，以计算对应的网络地址，从而用判断两个IP地址是否属于同一个网段
	if(trim($ip) == "" || trim($netmask) == "")
		return false;
	$ip_array = split(".",$ip);
	$netmask_array = split(".",$netmask);
	$netaddress="";
	$i=0;
	foreach($ip_array as $key => $value ){
			$netaddress .= $value & $netmask_array[$i];
			$i++;
	}
	return($netaddress);
}
...
if(isset($_REQUEST["action"])){
	...
	if(strtolower(trim($_REQUEST["action"])) === "ks"){    //KS信息处理
		  ...
		  $COMMANDREPLACE_ARRAY["{OSHOMEURL}"] = $pxeoshomeuri;
	...
	if(strtolower(trim($_REQUEST["action"])) === "commands"){     //处理命令请求
			...
			if($command_type === "finshed_install"){
		  	 ...
		  	 $result_array = mysql_fetch_array($result);
		  	 $pxenetname = $result_array[0];
		  	 $pxeinstallmac = $result_array[1];
		  ...
		  $result_array = mysql_fetch_array($result);
		  $COMMANDREPLACE_ARRAY["{DATATIME}"] = @date("YmdHis",time());
			$COMMANDREPLACE_ARRAY["{NETNAME}"] = $result_array[0];
			$COMMANDREPLACE_ARRAY["{HOSTNAME}"] = $result_array[1];
			$COMMANDREPLACE_ARRAY["{DNS1}"] = $result_array[2];
			$COMMANDREPLACE_ARRAY["{DNS2}"] = $result_array[3];
			$COMMANDREPLACE_ARRAY["{GATEWAY}"] = $result_array[4];
			$COMMANDREPLACE_ARRAY["{OSVER}"] = $osver;
 			$COMMANDREPLACE_ARRAY["{INSTALLMAC}"] =strtolower(str_replace(':',"-",trim($result_array[5])));
 			$COMMANDREPLACE_ARRAY["{HOSTID}"] = $hostid;
 			$COMMANDREPLACE_ARRAY["{INSTALLPASSWD}"] = $result_array[6];
			$query = "select pxeoshomeuri,pxekstemplatepath,pxeksuri,pxecommandtemplatepath from pxeconfig where pxenetname='".$COMMANDREPLACE_ARRAY["{NETNAME}"]."' and pxestatus=0";
 			if(!$result=mysql_db_query($DB["DATABASE"],$query,$DB_CONN)){
 				echo $query."<br>";
		  	echo mysql_error()."<br>";
		  	exit;
			...
			$result_array = mysql_fetch_array($result);
			$COMMANDREPLACE_ARRAY["{OSHOMEURL}"] = $result_array[0];
		  $COMMANDREPLACE_ARRAY["{KSURL}"] = $result_array[2];
		  $pxecommandtemplatepath = $result_array[3];
		  $query =  "select pxeostype, pxeosuri, pxecommandsuri,pxeinstallscriptpath from pxeos where pxenetname='".$COMMANDREPLACE_ARRAY["{NETNAME}"]."' and pxeosver='".$COMMANDREPLACE_ARRAY["{OSVER}"]."' and pxestatus=0";
		  if(!$result=mysql_db_query($DB["DATABASE"],$query,$DB_CONN)){
		  	echo $query."<br>";
		  	echo mysql_error()."<br>";
		  	exit;
		  ...
		  $result_array=mysql_fetch_array($result);
		  $COMMANDREPLACE_ARRAY["{OSTYPE}"] = $result_array[0];
		  $COMMANDREPLACE_ARRAY["{OSIMGURL}"] = $COMMANDREPLACE_ARRAY["{OSHOMEURL}"].$result_array[1];
		  $COMMANDREPLACE_ARRAY["{INSTALL_SCRIPT}"] = $result_array[3].$INSTALL_SCRIPT;
		  $query ="select pxenicno,pxenicip,pxenicmask,pxenicenbled from pxenics where pxehostid=".$COMMANDREPLACE_ARRAY["{HOSTID}"]." and pxenicstatus=0 order by pxenicno";
 			if(!$result=mysql_db_query($DB["DATABASE"],$query,$DB_CONN)){
		  	 echo $query."<br>";
		  	 echo mysql_error()."<br>";
		  	 exit;
		  }
		  $COMMANDREPLACE_ARRAY["{BINDGATEWAYNICNO}"] = -1;
 			for($i=0;$i<4;$i++){
 				if(($result_array=mysql_fetch_array($result))){
 					 $COMMANDREPLACE_ARRAY["{IP".($i+1)."}"] = $result_array[1];
 					 $COMMANDREPLACE_ARRAY["{MASK".($i+1)."}"] = $result_array[2];
 					 $COMMANDREPLACE_ARRAY["{EANBLED".($i+1)."}"] = $result_array[3];
 					 if((alc_net_address($result_array[1],$result_array[2]) == alc_net_address($COMMANDREPLACE_ARRAY["{GATEWAY}"],$result_array[2])) && ($result_array[3] == 0)){
 					 		$COMMANDREPLACE_ARRAY["{BINDGATEWAYNICNO}"] = $i;
 					 }
 				}
 				else{
 					 $COMMANDREPLACE_ARRAY["{IP".($i+1)."}"] = "";
 					 $COMMANDREPLACE_ARRAY["{MASK".($i+1)."}"] = "";
 					 $COMMANDREPLACE_ARRAY["{EANBLED".($i+1)."}"] = 1;
 				}
 			}
		  $commandtplfile = $pxecommandtemplatepath.$COMMANDREPLACE_ARRAY["{OSVER}"]."/".$command_type.".tpl";
		  $scriptcontent = "";
		  if(!$fp=@fopen($commandtplfile,"r")){
			...
			while (($line_str = fgets($fp, 4096)) !== false) {
 				if(($line_str = str_relace_array($line_str,$COMMANDREPLACE_ARRAY)) !== false)

...
create table `devicetype` (
    ...
    `status` tinyint(1) default 1 COMMENT '0:disabled 1:normal',
    primary key(`typeid`)
)ENGINE=InnoDB DEFAULT CHARSET=utf8;
insert into `devicetype`(typeid,typename,typedesc,status) values (1,'Router','router',1),
...
insert into os(osid,osname,osbit,version,`desc`) values (1,'CentOS',64,'6.4','CentOS 6.4'),
...
create table `brandinfo` (
    ...
    `local_name` varchar(255) comment '品牌本地语言名称，如中文',
    `english_name` varchar(255) comment '品牌英文名称',
    `offical_site` varchar(255) comment '官方网站地址',
    `support_site` varchar(255) comment '技术支持网站地址',
    `tel1` varchar(20) comment '服务电话号码1',
    `tel1_mem` varchar(50) comment '标注服务电话号码1的作用',
    `tel2` varchar(20) comment '服务电话号码2',
    `tel2_mem` varchar(50) comment '标注服务电话号码2的作用',
    `tel3` varchar(20) comment '服务电话号码3',
    `tel3_mem` varchar(50) comment '标注服务电话号码3的作用',
    `desc` varchar(255) comment '品牌描述',
    primary key(`id`)
)ENGINE=InnoDB DEFAULT CHARSET=utf8 ;
insert into `brandinfo`(`id`,`local_name`,`english_name`,`offical_site`,`support_site`,`tel1`,`tel1_mem`,`tel2`,`tel2_mem`,`tel3`,`tel3_mem`,`desc`) values 
...
create table `bzhy_interfaces` (
   `id`  bigint(20) unsigned not null auto_increment comment 'MAC 地址的ID号',
    ...
    primary key(`id`),
    CONSTRAINT `c_hostid` FOREIGN KEY (`hostid`) REFERENCES `hosts` (`hostid`) ON DELETE CASCADE
)ENGINE=InnoDB DEFAULT CHARSET=utf8 ;

create table `bzhy_ips`	(
    `id`  bigint(20) unsigned not null auto_increment comment 'IP地址ID ',
    `hostid`  bigint(20) unsigned not null comment '主机ID，与hosts表中hostid关联',
    `macid` bigint(20) unsigned not null default 0 comment '接口ID，与bzhy_interfaces表中id关联',
    ...
    `netmask` varchar(50) default '' comment 'IP地址对应的子网掩码',
    `status` tinyint(1) default 1 comment 'IP地址的状态1启用 0为禁用',
    `description` varchar(255) default '' comment 'IP地址描述',
    primary key(`id`),
    CONSTRAINT `bzhy_ips_c_hostid` FOREIGN KEY (`hostid`) REFERENCES `hosts` (`hostid`) ON DELETE CASCADE
)ENGINE=InnoDB DEFAULT CHARSET=utf8 ;

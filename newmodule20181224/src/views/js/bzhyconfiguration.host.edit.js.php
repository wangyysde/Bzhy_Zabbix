...
<tr class="interfaceRow" id="hostInterfaceRow_#{iface.interfaceid}" data-interfaceid="#{iface.interfaceid}">
    <td class="interface-drag-control <?= ZBX_STYLE_TD_DRAG_ICON ?>">
		...
		<?=_('Name:') ?><input name="interfaces[#{iface.interfaceid}][name]" type="text"  maxlength="64" value="#{iface.name}"> 
                <?=_('IP:') ?><input name="interfaces[#{iface.interfaceid}][ip]" type="text"  maxlength="64" value="#{iface.ip}"> 
                <?=_('Mask:') ?><input name="interfaces[#{iface.interfaceid}][mask]" type="text"  maxlength="64" value="#{iface.mask}">
                <div class="<?= ZBX_STYLE_TABLE_FORMS_SEPARATOR ?>"> <?=_('Type:') ?>
                ...
                <input type="radio" id="interfaces[#{iface.interfaceid}][kind]" name="interfaces[#{iface.interfaceid}][type]" value="0" #{attrs.typedummy}><?=_("Dummy") ?>
                </div>  
            </div>
            <div>
            <div> <?=_('Enable:') ?>
             <input type="radio" id="interfaces[#{iface.interfaceid}][enable][agent]" name="enable[agent]" value="#{iface.interfaceid}" onClick='Javascript:chg_port_display(#{iface.interfaceid},"agent");'><?=_('Agent') ?>
             <span id="interfaces[#{iface.interfaceid}][agent][port]"  style="display:none;">
                <input type="text" id="interfaces[#{iface.interfaceid}][port][agent]" name="port[agent]" value="#{iface.port_agent}" style="width: 60px">
             </span>
            <input type="radio" id="interfaces[#{iface.interfaceid}][enable][snmp]" name="enable[snmp]"  value="#{iface.interfaceid}" onClick='Javascript:chg_bulk(#{iface.interfaceid});'><?=_('SNMP') ?> 
            <span id="interfaces[#{iface.interfaceid}][snmp][port]"  style="display:none;">
                <input type="text" id="interfaces[#{iface.interfaceid}][port][snmp]" name="port[snmp]" value="#{iface.port_snmp}" style="width: 60px">
                <input type="checkbox" id="interfaces[#{iface.interfaceid}][bulk]" name="bulk" value="1" >
                <label for="interfaces[#{iface.interfaceid}][bulk]"><?= _('Use bulk requests') ?></label>
             </span>
             
            <input type="radio" id="interfaces[#{iface.interfaceid}][enable][jmx]" name="enable[jmx]"  value="#{iface.interfaceid}" onClick='Javascript:chg_port_display(#{iface.interfaceid},"jmx");'><?=_('JMX') ?>
            <span id="interfaces[#{iface.interfaceid}][jmx][port]"  style="display:none;">
                <input type="text" id="interfaces[#{iface.interfaceid}][port][jmx]" name="port[jmx]" value="#{iface.port_jmx}" style="width: 60px">
             </span>
            <input type="radio" id="interfaces[#{iface.interfaceid}][enable][ipmi]" name="enable[ipmi]"  value="#{iface.interfaceid}" onClick='Javascript:chg_port_display(#{iface.interfaceid},"ipmi");'><?=_('IPMI') ?>    
            <span id="interfaces[#{iface.interfaceid}][ipmi][port]"  style="display:none;">
                <input type="text" id="interfaces[#{iface.interfaceid}][port][ipmi]" name="port[ipmi]" value="#{iface.port_ipmi}" style="width: 60px">
             </span>
...
<script type="text/javascript">
    var agent_port_id = 0,snmp_port_id = 0,jmx_port_id = 0 ,ipmi_port_id = 0;
    function chg_bulk(interfaceid){
        obj_port = document.getElementById("interfaces[" + interfaceid + "][snmp][port]");
        obj_port.style.display = "";
        if(snmp_port_id !== 0 && snmp_port_id !== interfaceid){
            obj_port = document.getElementById("interfaces[" + snmp_port_id + "][snmp][port]");
	...
	var hostInterfacesManager = (function() {
		...
		function createNewHostInterface(hostInterfaceType) {
			var newInterface = {
				...
				port_agent: ports['agent'],
                                port_snmp: ports['snmp'],
                                port_jmx: ports['jmx'],
                                port_ipmi: ports['ipmi'],
				ip: '127.0.0.1'
			};

...
require_once dirname(__FILE__).'/../js/bzhyconfiguration.host.edit.js.php';
...
if ($data['flags'] != ZBX_FLAG_DISCOVERY_CREATED) {
	...
	$ifTab = (new CTable())
		...
		->addRow((new CRow([
			(new CCol(
				(new CButton('addAgentInterface', _('Add')))
                ->setId('addAgentInterface')
...
$inventoryFormList->addRow(_('Type'),
    (new CComboBox('typeid',isset($data['deviceinfo']['typeid'])?$data['deviceinfo']['typeid']:"","",$items))
        ->setAttribute('autofocus', 'autofocus')
);
...
$inventoryFormList->addRow(_('Brand'),
    (new CComboBox('brandid',isset($data['deviceinfo']['brandid'])?$data['deviceinfo']['brandid']:"","",$items))
);
...
$inventoryFormList->addRow(_('Size'),
    (new CComboBox('size',isset($data['deviceinfo']['size'])?$data['deviceinfo']['size']:"","",$items))
);

$inventoryFormList->addRow(_('Model'),
        (new CTextBox('model', isset($data['deviceinfo']['model']) ? $data['deviceinfo']['model']:""))
         ->setWidth(ZBX_TEXTAREA_STANDARD_WIDTH)
    );

$inventoryFormList->addRow(_('Host Tag'),
        (new CTextBox('tag', isset($data['deviceinfo']['tag']) ? $data['deviceinfo']['tag']:""))
         ->setWidth(ZBX_TEXTAREA_STANDARD_WIDTH)
    );
$inventoryFormList->addRow(_('Inventory Tag'),
        (new CTextBox('inventory_tag', isset($data['deviceinfo']['inventory_tag']) ? $data['deviceinfo']['inventory_tag']:""))
         ->setWidth(ZBX_TEXTAREA_STANDARD_WIDTH)
    );

$inventoryFormList->addRow(_('SerialNo'),
        (new CTextBox('serialno', isset($data['deviceinfo']['serialno']) ? $data['deviceinfo']['serialno']:""))
         ->setWidth(ZBX_TEXTAREA_STANDARD_WIDTH)
    );

$inventoryFormList->addRow(_('ServiceNo'),
        (new CTextBox('serviceno', isset($data['deviceinfo']['serviceno']) ? $data['deviceinfo']['serviceno']:""))
         ->setWidth(ZBX_TEXTAREA_STANDARD_WIDTH)
    );
$inventoryFormList->addRow(_('HardInfo'),
        (new CTextBox('hardinfo', isset($data['deviceinfo']['hardinfo']) ? $data['deviceinfo']['hardinfo']:""))
         ->setWidth(ZBX_TEXTAREA_STANDARD_WIDTH)
    );

$inventoryFormList->addRow(_('PurchaseTime'), createDateMenu('createdate', isset($data['deviceinfo']['createdate'])?$data['deviceinfo']['createdate']:time(),null,false));

$inventoryFormList->addRow(_('WarrantySince'), createDateMenu('warrantystartdate', isset($data['deviceinfo']['warrantystartdate'])?$data['deviceinfo']['warrantystartdate']:time(),null,false));

$inventoryFormList->addRow(_('WarrantyTo'), createDateMenu('warrantyenddate', isset($data['deviceinfo']['warrantyenddate'])?$data['deviceinfo']['warrantyenddate']:time(),null,false));

$inventoryFormList->addRow(_('DNSList'),
        (new CTextBox('dns', isset($data['deviceinfo']['dns']) ? $data['deviceinfo']['dns']:""))
         ->setWidth(ZBX_TEXTAREA_STANDARD_WIDTH)
    );

$inventoryFormList->addRow(_('GateWay'),
        (new CTextBox('gw', isset($data['deviceinfo']['gw']) ? $data['deviceinfo']['gw']:""))
         ->setWidth(ZBX_TEXTAREA_STANDARD_WIDTH)
    );
...
$inventoryFormList->addRow(_('Room'),
    (new CComboBox('roomid',isset($data['deviceinfo']['roomid'])?$data['deviceinfo']['roomid']:"","",$items))
        ->setId('roomid')
...
$inventoryFormList->addRow(_('Box'),
    (new CComboBox('boxid',isset($data['deviceinfo']['boxid'])?$data['deviceinfo']['boxid']:"","",$items))
        ->setId('subboxid')
);

$inventoryFormList->addRow(_('Position'),
        (new CTextBox('position', isset($data['deviceinfo']['position']) ? $data['deviceinfo']['position']:""))
         ->setWidth(ZBX_TEXTAREA_STANDARD_WIDTH)
    );
...
$inventoryFormList->addRow(_('BelongFrom'),
    (new CComboBox('belongdeviceid',isset($data['deviceinfo']['belongdeviceid'])?$data['deviceinfo']['belongdeviceid']:"","",$items))
);
...
$inventoryFormList->addRow(_('OS'),
    (new CComboBox('osid',isset($data['deviceinfo']['osid'])?$data['deviceinfo']['osid']:"","",$items))
);

$data['selectedContactIds'] = isset($data['deviceinfo']['selectContact'])?array_keys($data['deviceinfo']['selectContact']):"";
$contactTB = new CTweenBox($frmHost, 'contacts', NULL, 10);

foreach ($data['allContacts'] as $id => $row){
    $contactTB->addItem($row['id'], $row['contact_name'], is_array($data['selectedContactIds'])?(in_array($row['id'], $data['selectedContactIds'])?'yes':'no'):'no' ,TRUE);
}
$inventoryFormList->addRow(_('Contacts'), $contactTB->get(_('In Contacts'), _('Other Contacts')));

$data['selectedFileIds'] = isset($data['deviceinfo']['SelectFile'])?array_keys($data['deviceinfo']['SelectFile']):"";
$fileTB = new CTweenBox($frmHost, 'files', NULL, 10);
foreach ($this->data['allFiles'] as $id => $row){
    $fileTB->addItem($row['id'], $row['file_title'], is_array($data['selectedFileIds'])?(in_array($row['id'], $data['selectedFileIds'])?'yes':'no'):'no' ,TRUE);
}

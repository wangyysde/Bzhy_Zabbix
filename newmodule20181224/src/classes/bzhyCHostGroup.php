...
class bzhyCHostGroup {
	...
	//	$userid = self::$userData['userid'];
		...
		$sqlParts = [
			'select'	=> [bzhyCBase::getObjectTable($this->objectName) => bzhyDB::getFieldIdByObject($this->objectName,'groupid')],
		...
		$options = bzhy_array_merge($defOptions, $params);

		// groupids
		if (!is_null($options['groupids'])) {
			bzhy_value2array($options['groupids']);
			$sqlParts['where']['groupid'] = bzhydbConditionInt(bzhyDB::getFieldIdByObject($this->objectName, "groupid"), $options['groupids']);
		}
		...
		if (!is_null($options['hostids'])) {
			...
			$sqlParts['where'][] = bzhydbConditionInt('hg.hostid', $options['hostids']);
		...
		if (!is_null($options['maintenanceids'])) {
			...
			$sqlParts['where'][] = bzhydbConditionInt('mg.maintenanceid', $options['maintenanceids']);
		...
		if ($options['monitored_hosts'] !== null) {
			...
			$sub_sql_parts['where'][] = bzhydbConditionInt('h.status', [HOST_STATUS_MONITORED]);
		}
		elseif ($options['real_hosts'] !== null) {
			$sub_sql_parts['from']['h'] = 'hosts h';
			$sub_sql_parts['where']['hg-h'] = 'hg.hostid=h.hostid';
			$sub_sql_parts['where'][] = bzhydbConditionInt('h.status', [HOST_STATUS_MONITORED, HOST_STATUS_NOT_MONITORED]);
		}
		elseif ($options['templated_hosts'] !== null) {
			$sub_sql_parts['from']['h'] = 'hosts h';
			$sub_sql_parts['where']['hg-h'] = 'hg.hostid=h.hostid';
			$sub_sql_parts['where'][] = bzhydbConditionInt('h.status', [HOST_STATUS_TEMPLATE]);
		}
		elseif ($options['with_hosts_and_templates'] !== null) {
			$sub_sql_parts['from']['h'] = 'hosts h';
			$sub_sql_parts['where']['hg-h'] = 'hg.hostid=h.hostid';
			$sub_sql_parts['where'][] = bzhydbConditionInt('h.status',
		...
		if ($options['with_items'] !== null) {
			...
			$sub_sql_parts['where'][] = bzhydbConditionInt('i.flags',
		...
		elseif ($options['with_monitored_items'] !== null) {
			...
			$sub_sql_parts['where']['hg-h'] = 'hg.hostid=h.hostid';
			$sub_sql_parts['where'][] = bzhydbConditionInt('h.status', [HOST_STATUS_MONITORED]);
			$sub_sql_parts['where'][] = bzhydbConditionInt('i.status', [ITEM_STATUS_ACTIVE]);
			$sub_sql_parts['where'][] = bzhydbConditionInt('i.flags',
				[ZBX_FLAG_DISCOVERY_NORMAL, ZBX_FLAG_DISCOVERY_CREATED]
			);
		}
		elseif ($options['with_simple_graph_items'] !== null) {
			$sub_sql_parts['from']['i'] = 'items i';
			$sub_sql_parts['where']['hg-i'] = 'hg.hostid=i.hostid';
			$sub_sql_parts['where'][] = bzhydbConditionInt('i.value_type', [ITEM_VALUE_TYPE_FLOAT, ITEM_VALUE_TYPE_UINT64]);
			$sub_sql_parts['where'][] = bzhydbConditionInt('i.status', [ITEM_STATUS_ACTIVE]);
			$sub_sql_parts['where'][] = bzhydbConditionInt('i.flags',
				[ZBX_FLAG_DISCOVERY_NORMAL, ZBX_FLAG_DISCOVERY_CREATED]
			);
		...
		if ($options['with_triggers'] !== null) {
			...
			$sub_sql_parts['where'][] = bzhydbConditionInt('t.flags',
				[ZBX_FLAG_DISCOVERY_NORMAL, ZBX_FLAG_DISCOVERY_CREATED]
			);
		}
		elseif ($options['with_monitored_triggers'] !== null) {
			...
			$sub_sql_parts['where'][] = bzhydbConditionInt('h.status', [HOST_STATUS_MONITORED]);
			$sub_sql_parts['where'][] = bzhydbConditionInt('i.status', [ITEM_STATUS_ACTIVE]);
			$sub_sql_parts['where'][] = bzhydbConditionInt('t.status', [TRIGGER_STATUS_ENABLED]);
			$sub_sql_parts['where'][] = bzhydbConditionInt('t.flags',
				[ZBX_FLAG_DISCOVERY_NORMAL, ZBX_FLAG_DISCOVERY_CREATED]
			);
		}
		...
		elseif ($options['with_monitored_httptests'] !== null) {
			$sub_sql_parts['from']['ht'] = 'httptest ht';
			$sub_sql_parts['where']['hg-ht'] = 'hg.hostid=ht.hostid';
			$sub_sql_parts['where'][] = bzhydbConditionInt('ht.status', [HTTPTEST_STATUS_ACTIVE]);
		}
		...
		if ($options['with_graphs'] !== null) {
			...
			$sub_sql_parts['where'][] = bzhydbConditionInt('gr.flags',
				[ZBX_FLAG_DISCOVERY_NORMAL, ZBX_FLAG_DISCOVERY_CREATED]
			);
		}
		...
		if (is_array($options['filter'])) {
			bzhyDB::dbFilter('groups g', $options, $sqlParts);
		}

		// search
		if (is_array($options['search'])) {
			bzhyDB::dbSearch('groups g', $options, $sqlParts);
		}

		// limit
		if (ctype_digit(strval($options['limit'])) && $options['limit']) {
	...
	public function create(array $groups) {
		...
		foreach ($groups as $group) {
			$new_groupids = bzhyDB::insert('groups', [$group]);
	...
	public function delete(array $groupids, $nopermissions = false) {
		if (empty($groupids)) {
                    bzhyCBase::exception(ZBX_API_ERROR_PARAMETERS, _('Empty input parameter.'));
		}
		sort($groupids);
	...
	protected function validateCreate(array $groups) {
		...
		foreach ($groups as $group) {
			...
			if (!is_array($group)) {
                            bzhyCBase::exception(ZBX_API_ERROR_PARAMETERS, _('Incorrect arguments passed to function.'));
			}

			// Check required parameters.
			$missing_keys =  array_diff(['name'], array_keys($group));
			if ($missing_keys) {
                            bzhyCBase::exception(ZBX_API_ERROR_PARAMETERS,
			...
			// Check if host group name is valid.
                        if(!$this->validate($group['name'])){
                            bzhyCBase::exception(ZBX_API_ERROR_PARAMETERS,
					_s('Incorrect value for field "%1$s".', 'name')
				);
			}
                        if (array_key_exists('internal', $group)){
                            $error = vsprintf($error, ['internal', $group['name']]);
                            bzhyCBase::exception(ZBX_API_ERROR_PARAMETERS,$error);
                        }
		}

		// Check for duplicate names.
		$duplicate = findDuplicate($groups, 'name');
		if ($duplicate) {
                    bzhyCBase::exception(ZBX_API_ERROR_PARAMETERS,
		...
		$db_groups = bzhyDB::select('groups', [
			'output' => ['name'],
		...
		if ($check_names) {
			...
			foreach ($check_names as $group) {
				if (array_key_exists($group['name'], $db_group_names)
					...
					bzhyCBase::exception(ZBX_API_ERROR_PARAMETERS, _s('Host group "%1$s" already exists.', $group['name']));
				}
			}
		}
	...
	protected function validateMassAdd(array $data) {
		...
		if ($hostIds) {
			...
			$this->validateHostsPermissions($hostIds, $dbHosts);
                        ...
                        if($hosts){
                            $host = reset($hosts);
                            bzhyCBase::exception(ZBX_API_ERROR_PERMISSIONS,
				 _('Cannot update groups for discovered host "%1$s".',$host['host'])
			);
                        }
		...
		if ($templateIds) {
			$dbTemplates = bzhyAPI::Template()->get([
				'output' => ['templateid'],
				'selectGroups' => ['groupid'],
	...
	protected function validateMassUpdate(array $data) {
		...
		if (!$dbGroups) {
			bzhyCBase::exception(ZBX_API_ERROR_PERMISSIONS, _('No permissions to referred object or it does not exist!'));
		}
		...
		if (!$groupIdsToAdd && $groupIdsToRemove) {
			...
			if (count($objectIds) != count($unlinkableObjectIds)) {
				bzhyCBase::exception(ZBX_API_ERROR_PARAMETERS, _('One of the objects is left without a host group.'));
			}
		}
	...
	if ($options['selectDiscoveryRule'] !== null && $options['selectDiscoveryRule'] != API_OUTPUT_COUNT) {
		...
		' WHERE '.bzhydbConditionInt('gd.groupid', $groupIds).
    ...
    */
    public function validate($name) {
        if (!is_string($name)) {
            bzhyCBase::exception(ZBX_API_ERROR_PARAMETERS, _('must be a string'));
            return false;
	}

	if ($name === '') {
            bzhyCBase::exception(ZBX_API_ERROR_PARAMETERS, _('cannot be empty'));
            return false;
	}

	if ($name[0] === '/' || substr($name, -1) === '/' || strpos($name, '//') !== false
            || strpos($name, '*') !== false) {
            bzhyCBase::exception(ZBX_API_ERROR_PARAMETERS, _('invalid group name "%1$s"',$name));
            return false;
	}

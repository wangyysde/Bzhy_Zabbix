 ...
 */                         
class bzhyCHost extends bzhyCHostGeneral {
   
    protected $extra_objects = ['idc_room','file','contact','idc_box','device',
        'hostinterface','hostgroup','usermacro','template','proxy'];
    
    protected $objectName ='host';
    
    public function __construct() {
        if(!zbx_empty($this->extra_objects) && is_array($this->extra_objects)){
            bzhyCBase::addObjects($this->extra_objects);
        }
    }
	...
	public function get($options = []) {
		...
		$sqlParts = [
			'select'	=> [ bzhyCBase::getObjectTable($this->objectName) => bzhyDB::getFieldIdByObject($this->objectName,'hostid')],
			'from'		=> [bzhyDB::getFromByObject($this->objectName)],
			'where'		=> ['flags' =>  bzhyDB::getFieldIdByObject($this->objectName,'flags').' IN ('.ZBX_FLAG_DISCOVERY_NORMAL.','.ZBX_FLAG_DISCOVERY_CREATED.')'],
			'group'		=> [],
			'order'		=> [],
			'limit'		=> null
		];
		...
		if (!is_null($options['groupids'])) {
			bzhy_value2array($options['groupids']);

			$sqlParts['from']['hosts_groups'] = 'hosts_groups hg';
			$sqlParts['where'][] = bzhydbConditionInt('hg.groupid', $options['groupids']);
			$sqlParts['where']['hgh'] = 'hg.hostid='.bzhyDB::getFieldIdByObject($this->objectName,'hostid');
		...
		if (!is_null($options['proxyids'])) {
			bzhy_value2array($options['proxyids']);

			$sqlParts['where'][] = bzhydbConditionInt('h.proxy_hostid', $options['proxyids']);
		}

		// templateids
		if (!is_null($options['templateids'])) {
			bzhy_value2array($options['templateids']);

			$sqlParts['from']['hosts_templates'] = 'hosts_templates ht';
			$sqlParts['where'][] = bzhydbConditionInt('ht.templateid', $options['templateids']);
			$sqlParts['where']['hht'] = bzhyDB::getFieldIdByObject($this->objectName,'hostid').'=ht.hostid';
		...
		if (!is_null($options['interfaceids'])) {
			...
			$sqlParts['where'][] = bzhydbConditionInt('hi.interfaceid', $options['interfaceids']);
			$sqlParts['where']['hi'] = bzhyDB::getFieldIdByObject($this->objectName,'hostid').'=hi.hostid';
		}
		...
		if (!is_null($options['triggerids'])) {
			...
			$sqlParts['where']['hi'] = 'h.hostid='.bzhyDB::getFieldIdByObject($this->objectName,'hostid');
			$sqlParts['where']['fi'] = 'f.itemid=i.itemid';
		}

		// httptestids
		if (!is_null($options['httptestids'])) {
			bzhy_value2array($options['httptestids']);

			$sqlParts['from']['httptest'] = 'httptest ht';
			$sqlParts['where'][] = bzhydbConditionInt('ht.httptestid', $options['httptestids']);
			$sqlParts['where']['aht'] = 'ht.hostid='.bzhyDB::getFieldIdByObject($this->objectName,'hostid');
		}

		// graphids
		if (!is_null($options['graphids'])) {
			bzhy_value2array($options['graphids']);

			$sqlParts['from']['graphs_items'] = 'graphs_items gi';
			$sqlParts['from']['items'] = 'items i';
			$sqlParts['where'][] = bzhydbConditionInt('gi.graphid', $options['graphids']);
		...
		if (!is_null($options['applicationids'])) {
			bzhy_value2array($options['applicationids']);

			$sqlParts['from']['applications'] = 'applications a';
			$sqlParts['where'][] = bzhydbConditionInt('a.applicationid', $options['applicationids']);
			$sqlParts['where']['ah'] = 'a.hostid='.bzhyDB::getFieldIdByObject($this->objectName,'hostid');
		}

		// dserviceids
		if (!is_null($options['dserviceids'])) {
			bzhy_value2array($options['dserviceids']);

			$sqlParts['from']['dservices'] = 'dservices ds';
			$sqlParts['from']['interface'] = 'interface i';
			$sqlParts['where'][] = bzhydbConditionInt('ds.dserviceid', $options['dserviceids']);
		...
		if (!is_null($options['maintenanceids'])) {
			bzhy_value2array($options['maintenanceids']);

			$sqlParts['from']['maintenances_hosts'] = 'maintenances_hosts mh';
			$sqlParts['where'][] = bzhydbConditionInt('mh.maintenanceid', $options['maintenanceids']);
			$sqlParts['where']['hmh'] = bzhyDB::getFieldIdByObject($this->objectName,'hostid').'=mh.hostid';
		...
		if (!is_null($options['monitored_hosts'])) {
			$sqlParts['where']['status'] = bzhyDB::getFieldIdByObject($this->objectName,'status').'='.HOST_STATUS_MONITORED;
		}
		elseif (!is_null($options['templated_hosts'])) {
			$sqlParts['where']['status'] = bzhyDB::getFieldIdByObject($this->objectName,'status').' IN ('.HOST_STATUS_MONITORED.','.HOST_STATUS_NOT_MONITORED.','.HOST_STATUS_TEMPLATE.')';
		}
		elseif (!is_null($options['proxy_hosts'])) {
			$sqlParts['where']['status'] = bzhyDB::getFieldIdByObject($this->objectName,'status').' IN ('.HOST_STATUS_PROXY_ACTIVE.','.HOST_STATUS_PROXY_PASSIVE.')';
		}
		else {
			$sqlParts['where']['status'] = bzhyDB::getFieldIdByObject($this->objectName,'status').' IN ('.HOST_STATUS_MONITORED.','.HOST_STATUS_NOT_MONITORED.')';
		}
		...
		if (!is_null($options['with_items'])) {
			$sqlParts['where'][] = 'EXISTS ('.
					'SELECT NULL'.
					' FROM items i'.
					' WHERE '.bzhyDB::getFieldIdByObject($this->objectName,'hostid').'=i.hostid'.
		...
		if (!empty($options['with_httptests'])) {
			$sqlParts['where'][] = 'EXISTS (SELECT NULL FROM httptest ht WHERE ht.hostid='.bzhyDB::getFieldIdByObject($this->objectName,'hostid').')';
		}
		elseif (!empty($options['with_monitored_httptests'])) {
			$sqlParts['where'][] = 'EXISTS ('.
				'SELECT NULL'.
				' FROM httptest ht'.
				' WHERE '.bzhyDB::getFieldIdByObject($this->objectName,'hostid').'=ht.hostid'.
		...
		if (!is_null($options['with_graphs'])) {
			$sqlParts['where'][] = 'EXISTS ('.
					...
					' WHERE i.hostid='.bzhyDB::getFieldIdByObject($this->objectName,'hostid').
		...
		if (!is_null($options['with_applications'])) {
			$sqlParts['from']['applications'] = 'applications a';
			$sqlParts['where'][] = 'a.hostid='.bzhyDB::getFieldIdByObject($this->objectName,'hostid');
		}

		// withInventory
		if (!is_null($options['withInventory']) && $options['withInventory']) {
			$sqlParts['where'][] = ' '.bzhyDB::getFieldIdByObject($this->objectName,'hostid').' IN ('.
					' SELECT hin.hostid'.
		...
		if ($options['searchInventory'] !== null) {
			...
			$sqlParts['where']['hii'] = bzhyDB::getFieldIdByObject($this->objectName,'hostid').'=hii.hostid';

			bzhy_db_search('host_inventory hii',
				[
		...
		if (is_array($options['filter'])) {
			...
			if (bzhyDB::dbFilter('interface hi', $options, $sqlParts)) {
				$sqlParts['from']['interface'] = 'interface hi';
				$sqlParts['where']['hi'] = bzhyDB::getFieldIdByObject($this->objectName,'hostid').'=hi.hostid';
			}
		}
		...
		$sqlParts = bzhyDB::applyQuerySortOptions(bzhyDB::getFieldIdByObject($this->objectName,'hostid'), bzhyCBase::getTableAliasByObject($this->objectName), $options, $sqlParts);
    ...
    */
    public function create($hosts) {
		...
		if ((array_key_exists('tls_connect', $host) && $host['tls_connect'] != HOST_ENCRYPTION_CERTIFICATE)
                ...
                echo "222222";
		$hostid = bzhyDB::insert(bzhyCBase::getObjectTable($this->objectName), [$host]);
		$hostid = reset($hostid);
		$host['hostid'] = $hostid;
		$hostids[] = $hostid;
                echo "3333333";
		...
		bzhyDB::insert('hosts_groups', $groupsToAdd);
                echo "444444";
		$options = [
                    'hosts' => $host
		];
		...
		if (isset($host['interfaces']) && !is_null($host['interfaces'])) {
                ...
                echo "555555";
		if (!$result) {
                    bzhyCBase::exception(BZHY_API_ERROR_INTERNAL,_('Create host error'),__FILE__,__LINE__);
		}
                
                //bzhy interface and IPS
                $inventoryip = $host['bzhyinventory'];
                foreach ($host["bzhyinterfaces"] as $key => $bzhyinterface){
                    $bzhyinterfaces['hostid'] = $hostid;
                    $bzhyinterfaces = zbx_array_merge($bzhyinterfaces, $bzhyinterface);
                    $bzhy_interfaceid = bzhyDB::insert('bzhy_interfaces', [$bzhyinterfaces]);
                    $ips['hostid'] =  $hostid;
                    $bzhy_interfaceid = reset($bzhy_interfaceid);
                    $ips['macid'] = $bzhy_interfaceid;
                    $ips['type'] = HOST_IP_TYPE_IP;
                    $ips['family'] = HOST_IP_FAMILY_VER4;
                    $ips['ip'] = $bzhyinterface['ip'];
                    $ips['netmask'] = $bzhyinterface['mask'];
                    $ips['status'] = STATUS_NORMAL;
                    $ips['description'] = '';
                    $ipid = bzhyDB::insert('bzhy_ips', [$ips]);
                    echo "666666";
                }
                if(!zbx_empty($inventoryip['gw'])){
                    $gws = explode(';',$inventoryip['gw'] );
                    foreach ($gws as $gw){
                        $ips['hostid'] =  $hostid;
                        $ips['macid'] = $bzhy_interfaceid;
                        $ips['type'] = HOST_IP_TYPE_GW;
                        $ips['family'] = HOST_IP_FAMILY_VER4;
                        $ips['ip'] = $gw;
                        $ips['netmask'] = '';
                        $ips['status'] = STATUS_NORMAL;
                ...
                $bzhyinventoryid = bzhyDB::insert('bzhy_host_inventory', [$bzhyinventory]);
                $bzhyinventoryid = reset($bzhyinventoryid);
                echo "999999";
                if(($Fields = bzhyDB::getTableFields($this->inventoryTable)) == FALSE){
                    throw new Exception(_('Can not get inventory table fields'));
                }
                $files = $bzhyinventory['files'];
                if(!zbx_empty($files)){
                    $relatedFile['FileIds'] = $files;
                    $relatedFile['RelatedTable'] = $this->inventoryTable;
                    $relatedFile['RelatedField'] = $Fields['id'];
                    $relatedFile['RelatedValue'] = $bzhyinventoryid;
                    $cFile = bzhyAPI::File();
                    $cFile->data = $relatedFile;
                    echo "10101010";
                    if(!$cFile->insertRelatedObjects()){
                        throw new Exception(_('Add related files error!'));
                    }
                }
                ...
                if(!zbx_empty($contacts)){
                    ...
                    $relatedContact['RelatedTable'] = $this->inventoryTable;
                    $relatedContact['RelatedField'] = $Fields['id'];
                    $relatedContact['RelatedValue'] = $bzhyinventoryid;
                    $cContact = bzhyAPI::Contact();
                    $cContact->data = $relatedContact;
                    echo "AAAAAAAAAA";
                    if(!$cContact->insertRelatedObjects()){
                        throw new Exception(_('Add related contacts error!'));
                    }
                    echo "BBBBBBBBBB";
                }
            }
            ...
            bzhyCBase::exception($ex->getCode(),$ex->getMessage(),__FILE__,__LINE__);
        }
    ...
    */
    public function massAdd(array $data) {
		...
		return parent::massAdd($data);
            ...
            bzhyCBase::exception(ZBX_API_ERROR_INTERNAL,$ex->getMessage(),__FILE__,__LINE__);
        }
    }
    ...
    */
    protected function validateEncryption(array $hosts) {
		...
		bzhyCBase::exception(BZHY_API_ERROR_PARAMETERS, _s('Incorrect value for field "%1$s": %2$s.', 'tls_connect',
		...
		bzhyCBase::exception(BZHY_API_ERROR_PARAMETERS, _s('Incorrect value for field "%1$s": %2$s.', 'tls_accept',
		...
		if (!preg_match('/^([0-9a-f]{2})+$/i', $host['tls_psk'])) {
                    bzhyCBase::exception(BZHY_API_ERROR_PARAMETERS, _(
    ...
    */
	...
	foreach ($hosts as &$host) {
            ...
            // Validate "host" field.
            if (!preg_match('/^'.BZHY_PREG_HOST_FORMAT.'$/', $host['host'])) {
                bzhyCBase::exception(BZHY_API_ERROR_PARAMETERS,
		...
		,__FILE__,__LINE__);
            ...
            if (!array_key_exists('groups', $host) || !is_array($host['groups']) || !$host['groups']) {
                bzhyCBase::exception(BZHY_API_ERROR_PARAMETERS, _s('No groups for host "%1$s".', $host['host']),__FILE__,__LINE__);
            }

            $groupids = array_merge($groupids, bzhy_objectValues($host['groups'], 'groupid'));
	}
	unset($host);

	// Check for duplicate "host" and "name" fields.
	$duplicate = findDuplicate($hosts, 'host');
	if ($duplicate) {
            bzhyCBase::exception(BZHY_API_ERROR_PARAMETERS,
	...
	$duplicate = findDuplicate($hosts, 'name');
	if ($duplicate) {
            bzhyCBase::exception(BZHY_API_ERROR_PARAMETERS,
	...
	if ($groupids) {
            $db_groups = bzhyAPI::HostGroup()->get([
                'output' => ['groupid'],
		'groupids' => $groupids,
		'editable' => true,
		'preservekeys' => true
	...
	foreach ($hosts as $host) {
		...
		bzhyCBase::exception(BZHY_API_ERROR_PARAMETERS, _s('No interfaces for host "%s".', $host['host']),__FILE__,__LINE__);
            }

            if (array_key_exists('status', $host)) {
                if (!is_string($host['status']) && !is_int($host['status'])) {
                   bzhyCBase::exception(BZHY_API_ERROR_PARAMETERS,  _s('Incorrect status for host "%s".',$host['host']),__FILE__,__LINE__);
                }
	...
	foreach ($hosts_exists as $host_exists) {
		...
		bzhyCBase::exception(BZHY_API_ERROR_PARAMETERS,
                    _s('Host with the same name "%s" already exists.', $host_exists['host'],__FILE__,__LINE__)
		);
            }

            if (array_key_exists($host_exists['name'], $host_names['name'])) {
		bzhyCBase::exception(BZHY_API_ERROR_PARAMETERS,
                    _s('Host with the same visible name "%s" already exists.', $host_exists['name'],__FILE__,__LINE__)
		,__FILE__,__LINE__);
            }
	}
                
        $bzhyTemplate = bzhyAPI::Template(); 
	$templates_exists = $bzhyTemplate->get([
		...
		bzhyCBase::exception(ZBX_API_ERROR_PARAMETERS,
                    _s('Template with the same visible name "%s" already exists.', $template_exists['name'],__FILE__,__LINE__)
                );
            }
	}

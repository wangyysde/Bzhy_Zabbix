 ...
 */
abstract class bzhyCGraphGeneral {
	...
	public function update(array $graphs) {
		$graphs = bzhy_toArray($graphs);
		$graphIds = bzhy_objectValues($graphs, 'graphid');

		$graphs = bzhyDB::extendObjects('graph', $graphs,
			['name', 'graphtype', 'ymin_type', 'ymin_itemid', 'ymax_type', 'ymax_itemid', 'yaxismin', 'yaxismax']
		);

		$dbGraphs = $this->get([
			'output' => BZHYAPI_OUTPUT_EXTEND,
			'selectGraphItems' => BZHYAPI_OUTPUT_EXTEND,
			'graphids' => $graphIds,
			'editable' => true,
			'preservekeys' => true,
			'inherited' => false
	...
	protected function updateReal(array $graph, array $dbGraph) {
		...
		$dbGitemIds = zbx_toHash(bzhy_objectValues($dbGitems, 'gitemid'));
		...
		foreach ($graphs as $graph) {
			...
			if (isset($graph['gitems'])) {
				...
				$graphHosts = API::Host()->get([
					'itemids' => bzhy_objectValues($graph['gitems'], 'itemid'),
			...
			if (isset($graph['gitems'])) {
				...
				if (HOST_STATUS_TEMPLATE == $host['status']) {
					...
					if ($itemIds) {
						...
						$itemHosts = array_unique(bzhy_objectValues($itemHosts, 'hostid'));

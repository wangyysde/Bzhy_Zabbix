 ...
 */
class bzhyCTemplate extends bzhyCHostGeneral {

	protected $sortColumns = ['hostid', 'host', 'name'];
        
        protected $objectName ='template';
	...
	public function get($options = []) {
		...
		$sqlParts = [
			...
			'where'		=> [bzhyDB::getFieldIdByObject($this->objectName,'status').'='.HOST_STATUS_TEMPLATE],
			'group'		=> [],
			'order'		=> [],
			'limit'		=> null
		];
		...
		if (!is_null($options['templateids'])) {
			...
			$sqlParts['where']['templateid'] = bzhydbConditionInt(bzhyDB::getFieldIdByObject($this->objectName,'hostid'), $options['templateids']);
		}

		// parentTemplateids
		if (!is_null($options['parentTemplateids'])) {
			bzhy_value2array($options['parentTemplateids']);

			$sqlParts['from']['hosts_templates'] = 'hosts_templates ht';
			$sqlParts['where'][] = bzhydbConditionInt('ht.templateid', $options['parentTemplateids']);
		...
		if (!is_null($options['hostids'])) {
			...
			$sqlParts['from']['hosts_templates'] = 'hosts_templates ht';
			$sqlParts['where'][] = bzhydbConditionInt('ht.hostid', $options['hostids']);
			$sqlParts['where']['hht'] = bzhyDB::getFieldIdByObject($this->objectName,'hostid').'=ht.templateid';
		...
		if (!empty($options['with_httptests'])) {
			$sqlParts['where'][] = 'EXISTS (SELECT ht.httptestid FROM httptest ht WHERE ht.hostid='.bzhyDB::getFieldIdByObject($this->objectName,'hostid');
		}

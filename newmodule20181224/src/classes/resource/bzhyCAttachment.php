    ...
    */    
    ...
    public function get($options = []) {
        ...
        if (!is_null($options['ids'])) {
            ...
            $sqlParts['where']['file'] = bzhydbConditionInt( bzhyDB::getFieldIdByObject($this->objectName,'id'), $options['ids']);
	}
        if(!is_null($options['status'])){
            zbx_value2array($options['status']);
            $sqlParts['where']['file_status'] = bzhydbConditionInt( bzhyDB::getFieldIdByObject($this->objectName,'file_status'), $options['status']);
        }
        if (!is_null($options['object_ids']) && !is_null($options['object_table'])) {
            ...
            $sqlParts['where'][] = bzhydbConditionInt('fo.object_value', $options['object_ids']);
            $sqlParts['where']['object_file'] = 'fo.file_id = '.bzhyDB::getFieldIdByObject($this->objectName,'id');

...
class bzhyCProxy {
	...
	public function get($options = []) {
		...
		$sqlParts = [
			'select'	=> [bzhyDB::getPkByObject($this->objectName) => bzhyDB::getFieldIdByObject($this->objectName,'hostid')],
			'from'		=> [bzhyDB::getFromByObject($this->objectName)],
			'where'		=> [bzhyDB::getFieldIdByObject($this->objectName,'status').' IN ('.HOST_STATUS_PROXY_ACTIVE.','.HOST_STATUS_PROXY_PASSIVE.')'],
			'order'		=> [],
			'limit'		=> null
		];
		...
		if (!is_null($options['proxyids'])) {
			...
			$sqlParts['where'][] = bzhydbConditionInt(bzhyDB::getFieldIdByObject($this->objectName,'hostid'), $options['proxyids']);
		}
		...
		if ($options['output'] == API_OUTPUT_EXTEND) {
			$sqlParts['select']['hostid'] = bzhyDB::getFieldIdByObject($this->objectName,'hostid');
			$sqlParts['select']['host'] = bzhyDB::getFieldIdByObject($this->objectName,'host');
			$sqlParts['select']['status'] = bzhyDB::getFieldIdByObject($this->objectName,'status');
			$sqlParts['select']['lastaccess'] = bzhyDB::getFieldIdByObject($this->objectName,'lastaccess');
		}

		// countOutput
		if (!is_null($options['countOutput'])) {
			$options['sortfield'] = '';
			$sqlParts['select'] = ['COUNT(DISTINCT '.bzhyDB::getFieldIdByObject($this->objectName,'hostid').') AS rowscount'];
		}
	...
	public function delete(array $proxyIds) {
		...
		$dbProxies = DBselect(
			...
			' FROM hosts h'.
			' WHERE '.bzhydbConditionInt('h.hostid', $proxyIds)
		);
	...
	protected function checkUsedInDiscoveryRule(array $proxyIds) {
		$dRule = DBfetch(DBselect(
			...
			' FROM drules dr'.
			' WHERE '.bzhydbConditionInt('dr.proxy_hostid', $proxyIds),
			1
		));
		if ($dRule) {
	...
	protected function checkUsedForMonitoring(array $proxyIds) {
		$host = DBfetch(DBselect(
			...
			' FROM hosts h'.
			' WHERE '.bzhydbConditionInt('h.proxy_hostid', $proxyIds),
			1
		));
		if ($host) {

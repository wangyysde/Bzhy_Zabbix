 ...
 */
abstract class bzhyCHostGeneral extends bzhyCHostBase {
	...
	public function massAdd(array $data) {
		...
		if (!empty($data['groups'])) {
                    bzhyCBase::getObject('hostgroup')->massADD([
			'hosts' => $data['hosts'],
		...
		if (!empty($data['templates_link'])) {
			...
			//if (!API::Host()->isWritable($allHostIds)) {
                        if (!bzhyCBase::getObject('host')->isWritable($allHostIds)) {
			...
			foreach ($data['macros'] as $hostMacro) {
				foreach ($allHostIds as $hostid) {
					...
					$hostMacrosToAdd[] = $hostMacro;
                        ...
                        bzhyCBase::getObject('usermacro')->create($hostMacrosToAdd);
			//API::UserMacro()->create($hostMacrosToAdd);
		}

		$ids = ['hostids' => $hostIds, 'templateids' => $templateIds];
                $Fields = bzhyDB::getTableFields('hosts');

...
class bzhyCProfile {
    ...
    protected static  $objectName ='profile';
        
    public static function init() {
        self::$userDetails = bzhyCWebUser::$data;
	self::$profiles = [];
        $Fields = bzhyDB::getFields(bzhyCBase::getObjectTable(self::$objectName));
        self::$stringProfileMaxLength = $Fields['value_str']['length'];
        $query = 'SELECT '.bzhyDB::getFieldIdByObject(self::$objectName, '*').
            ' FROM '.bzhyDB::getFromByObject(self::$objectName).
            ' WHERE '.bzhyDB::getFieldIdByObject(self::$objectName, 'userid').'='.self::$userDetails['userid'].
            ' ORDER BY '.bzhyDB::getFieldIdByObject(self::$objectName, 'userid').','.bzhyDB::getFieldIdByObject(self::$objectName, 'profileid');
        
        $db_profiles = DBselect($query);
    ...
    */
    protected static function deleteValues($idx, array $idx2) {
        // remove from DB
	bzhyDB::delete(bzhyCBase::getObjectTable(self::$objectName), ['idx' => $idx, 'idx2' => $idx2, 'userid' => self::$userDetails['userid']]);
    }
    ...
    */
	...
	$values = [
        ...
        return bzhyDB::insert(bzhyCBase::getObjectTable(self::$objectName), [$values]);
    }
	...
	return DBexecute(
            'UPDATE '.bzhyCBase::getObjectTable(self::$objectName).' SET '.
	...
	switch ($type) {
            case BZHYPROFILE_TYPE_INT:
		$field = 'value_int';
		break;
            case BZHYPROFILE_TYPE_STR:
		$field = 'value_str';
                break;
            case BZHYPROFILE_TYPE_ID:
            default:
		$field = 'value_id';
        }
        return $field;

...
class bzhyCUser{
    ...
    * @param bool   $options['type']			filter by User type [USER_TYPE_ZABBIX_USER: 1, USER_TYPE_ZABBIX_ADMIN: 2, BZHYUSER_TYPE_SUPER_ADMIN: 3]
    ...
    */
    public function get($options = []) {
        $result = [];
        $sqlParts = [
            'select'	=> [bzhyCBase::getObjectTable(self::$objectName) => bzhyDB::getFieldIdByObject(self::$objectName,'userid')],
	...
	if ($options['userids'] !== null) {
            ...
            $sqlParts['where'][] = bzhydbConditionInt(bzhyDB::getFieldIdByObject(self::$objectName, 'userid'), $options['userids']);
	}
	...
	if ($options['usrgrpids'] !== null) {
            ...
            $sqlParts['where']['uug'] = bzhyDB::getFieldIdByObject(self::$objectName, 'userid').'=ug.userid';
	}
	// mediaids
	if ($options['mediaids'] !== null) {
            bzhy_value2array($options['mediaids']);
            $sqlParts['from']['media'] = 'media m';
            $sqlParts['where'][] = bzhydbConditionInt('m.mediaid', $options['mediaids']);
            $sqlParts['where']['mu'] = 'm.userid='.bzhyDB::getFieldIdByObject(self::$objectName,'userid');
	}
	// mediatypeids
	if ($options['mediatypeids'] !== null) {
            bzhy_value2array($options['mediatypeids']);
            $sqlParts['from']['media'] = 'media m';
            $sqlParts['where'][] = bzhydbConditionInt('m.mediatypeid', $options['mediatypeids']);
            $sqlParts['where']['mu'] = 'm.userid='.bzhyDB::getFieldIdByObject(self::$objectName,'userid');
	}
        // filter
	if (is_array($options['filter'])) {
            if (isset($options['filter']['passwd'])) {
		bzhyCBase::exception(BZHY_API_ERROR_PARAMETERS, _('It is not possible to filter by user password.'),__FILE__,__LINE__);
            }

            $this->dbFilter(bzhyDB::getFromByObject(self::$objectName), $options, $sqlParts);
        }
	// search
	if (is_array($options['search'])) {
            if (isset($options['search']['passwd'])) {
                bzhyCBase::exception(BZHY_API_ERROR_PARAMETERS, _('It is not possible to search by user password.'),__FILE__,__LINE__);
            }
	...
	*/
	if ($options['getAccess'] !== null) {
		...
		' WHERE '.bzhydbConditionInt('ug.userid', $userIds).
		' AND g.usrgrpid=ug.usrgrpid'.
		' GROUP BY ug.userid'
            );
            while ($userAccess = DBfetch($access)) {
                $result[$userAccess['userid']] = bzhy_array_merge($result[$userAccess['userid']], $userAccess);
            }
	}
	if ($result) {
	...
	protected function checkInput(&$users, $method) {
		...
		foreach ($users as &$user) {
			...
			// permissions
			if ($create) {
				if (self::$userData['type'] != BZHYUSER_TYPE_SUPER_ADMIN) {
			...
			elseif ($update) {
				...
				if (bccomp(self::$userData['userid'], $user['userid']) != 0 && self::$userData['type'] != BZHYUSER_TYPE_SUPER_ADMIN) {
			...
			if (isset($user['type']) && (BZHYUSER_TYPE_SUPER_ADMIN != self::$userData['type'])) {
	...
	$userIds = bzhy_objectValues($result, 'userid');
	// adding usergroups
	if ($options['selectUsrgrps'] !== null && $options['selectUsrgrps'] != BZHYAPI_OUTPUT_COUNT) {
            $relationMap = bzhyCBase::createRelationMap($result, 'userid', 'usrgrpid', 'users_groups');
            $dbUserGroups = bzhyAPI::UserGroup()->get([
	...
	// adding medias
	if ($options['selectMedias'] !== null && $options['selectMedias'] != BZHYAPI_OUTPUT_COUNT) {
            $userMedias = bzhyAPI::UserMedia()->get([
                'output' => outputExtend($options['selectMedias'], ['userid', 'mediaid']),
		'userids' => $userIds,
		'preservekeys' => true
            ]);
            $relationMap = bzhyCBase::createRelationMap($userMedias, 'userid', 'mediaid');
            $userMedias = bzhyCBase::unsetExtraFields($userMedias, ['userid', 'mediaid'], $options['selectMedias']);
            $result = $relationMap->mapMany($result, $userMedias, 'medias');
	}
	// adding media types
	if ($options['selectMediatypes'] !== null && $options['selectMediatypes'] != BZHYAPI_OUTPUT_COUNT) {
            $relationMap = bzhyCBase::createRelationMap($result, 'userid', 'mediatypeid', 'media');
            $mediaTypes = bzhyAPI::Mediatype()->get([

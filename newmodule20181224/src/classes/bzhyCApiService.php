...
class bzhyCApiService {
	...
	protected function applyPostSqlFiltering(array $elements, array $options) {
		...
		if (!is_null($options['groupCount'])) {
                    ...
                    $sqlParts['select'] = ['id',$objectField,'object_table','object_field','object_value','count(id) as objectcount'];
                }
                else{
            ...
            if(!isset( $sqlParts['select'])){
                $sqlParts['select'] = ['id',$objectField,'object_table','object_field','object_value'];
            }
            ...
            if(!is_null($options['countOutput']) && is_null($options['groupCount'])){
                ...
                while ($object = DBfetch($res)){
                    if(!is_null(($object_table=$object['object_table']))){
                        $object_type = $this->getObjectName($object_table);
                        $object_type_field = $this->getObjectNameField($object_table);
                        if(!isset($options['output']) || is_null($options['output']) || $null_output){
                            $options['output'] = [$object_type_field];
                            $null_output = TRUE;
                        }
                        $object_type_pk=$this->pk($object_table);
                        $object_type_value = $object['object_value'];
                        if(is_null(trim($object_type))){
                            self::exception(ZBX_API_ERROR_INTERNAL, _('Can not found object type!'));
                        }
                    }
                    else{
                        self::exception(ZBX_API_ERROR_INTERNAL, _('Can not found objects!'));
                    }
                    
                    //if(count(($ObjectInfos= API::getApi($object_table)->get(['ids'=>$object_type_value,"output"=>[implode(',',$options['output'])]])))>0){
                    if(count(($ObjectInfos= CBase::getObject($object_table)->get(['ids'=>$object_type_value,"output"=>[implode(',',$options['output'])]])))>0){
                 ...
                 //   $result['object_pk_value'][$object_table] = $object_type_value;
                    if(!is_null($options['countOutput']) && !is_null($options['groupCount'])){
        ...
        public function getObjectName($tableName = null) {
            if ($tableName) {
		$schema = $this->getTableSchema($tableName);
                if(!isset($schema['object_name'])){
                    $tableobject = include(dirname(__FILE__)."/../bzhyinclude/tableobject.inc.php");
                    if(isset($tableobject[$tableName]["object_name"])){
                        return $tableobject[$tableName]["object_name"];
                    }
                    else{
                        return $tableName;
                    }
        ...
        public function getObjectNameField($tableName = null) {
            if ($tableName) {
		$schema = $this->getTableSchema($tableName);
                if(!isset($schema['object_name_field'])){
                    $tableobject = include(dirname(__FILE__)."/../bzhyinclude/tableobject.inc.php");
                    if(isset($tableobject[$tableName]["object_name_field"])){
                        return $tableobject[$tableName]["object_name_field"];
                    }
                    else{
                        return "";
                    }
                }
 		return $schema['object_name_field'];
            }
            return FALSE;
	}

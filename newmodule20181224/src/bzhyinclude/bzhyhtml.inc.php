...
function bzhyitalic($str) {
	if (is_array($str)) {
		foreach ($str as $key => $val) {
			if (is_string($val)) {
				$em = new bzhyCTag('em', true);
				$em->addItem($val);
				$str[$key] = $em;
			}
		}
...
function bzhybold($str) {
	if (is_array($str)) {
		foreach ($str as $key => $val) {
			if (is_string($val)) {
				$str[$key] = new bzhyCTag('b', true, $val);
			}
		}
	...
	return new bzhyCTag('b', true, $str);
}

function bzhymake_decoration($haystack, $needle, $class = null) {
	$result = $haystack;
	...
	if ($pos !== false) {
		$start = bzhyCHtml::encode(mb_substr($haystack, 0, $pos));
		$end = bzhyCHtml::encode(mb_substr($haystack, $pos + mb_strlen($needle)));
		$found = bzhyCHtml::encode(mb_substr($haystack, $pos, mb_strlen($needle)));

		if (is_null($class)) {
			$result = [$start, bold($found), $end];
		}
		else {
			$result = [$start, (new bzhyCSpan($found))->addClass($class), $end];
		}
	}
...
function bzhynbsp($str) {
	return str_replace(' ', BZHYSPACE, $str);
}

function bzhyprepareUrlParam($value, $name = null) {
	if (is_array($value)) {
		$result = '';

		foreach ($value as $key => $param) {
			$result .= bzhyprepareUrlParam($param, isset($name) ? $name.'['.$key.']' : $key);
		}
	}
	else {
 ...
 * @return string
 */
function bzhyurl_param($param, $getFromRequest = true, $name = null) {
	if (is_array($param)) {
		if ($getFromRequest) {
...
function bzhyurl_params(array $params) {
	$result = '';
...
function bzhyBR() {
	return new bzhyCTag('br');
}

function bzhyget_icon($type, $params = []) {
	switch ($type) {
		case 'favourite':
			if (CFavorite::exists($params['fav'], $params['elid'], $params['elname'])) {
				$icon = (new CRedirectButton(BZHYSPACE, null))
		...
		case 'fullscreen':
			...
			if ($params['fullscreen'] == 0) {
				...
				$icon = (new CRedirectButton(BZHYSPACE, $url->getUrl()))
		...
		case 'dashconf':
			$icon = (new CRedirectButton(BZHYSPACE, 'dashconf.php'))
		...
		case 'screenconf':
			return (new CRedirectButton(BZHYSPACE, null))
				->addClass(ZBX_STYLE_BTN_CONF)
 ...
 * @return object
 */
function bzhyget_header_host_table($current_element, $hostid, $lld_ruleid = 0) {
	$options = [
		'output' => [
	...
	if ($is_template) {
		...
		if ($db_host['proxy_hostid'] != 0) {
			...
			$proxy_name = bzhyCHtml::encode($db_proxies[0]['host']).NAME_DELIMITER;
		}

		$name = $proxy_name.bzhyCHtml::encode($db_host['name']);

		switch ($db_host['status']) {
			case HOST_STATUS_MONITORED:
				if ($db_host['maintenance_status'] == HOST_MAINTENANCE_STATUS_ON) {
					$status = (new bzhyCSpan(_('In maintenance')))->addClass(ZBX_STYLE_ORANGE);
				}
				else {
					$status = (new bzhyCSpan(_('Enabled')))->addClass(ZBX_STYLE_GREEN);
				}
				break;
			case HOST_STATUS_NOT_MONITORED:
				$status = (new bzhyCSpan(_('Disabled')))->addClass(ZBX_STYLE_RED);
				break;
			default:
				$status = _('Unknown');
				break;
		...
		$host = new bzhyCSpan(new CLink($name, 'hosts.php?form=update&hostid='.$db_host['hostid']));
		...
		$list->addItem([
			new bzhyCSpan(new CLink(_('All hosts'), 'hosts.php?hostid='.$db_host['hostid'].url_param('groupid'))),
			'/',
			$host
		]);
		$list->addItem($status);
	...
	if ($lld_ruleid == 0) {
		...
		$discovery_rule = (new bzhyCSpan())->addItem(
			new CLink(
				bzhyCHtml::encode($db_discovery_rule['name']),
		...
		// trigger prototypes
		$trigger_prototypes = new bzhyCSpan([
		...
		// graph prototypes
		$graph_prototypes = new bzhyCSpan([
 ...
 * @return object
 */
function bzhyget_header_sysmap_table($sysmapid, $name, $fullscreen, $severity_min) {
	$list = (new CList())
		->addClass(ZBX_STYLE_OBJECT_GROUP)
		->addItem([
			(new bzhyCSpan())->addItem(new CLink(_('All maps'), 'sysmaps.php')),
			'/',
			(new bzhyCSpan())
	...
	if ($parent_sysmaps) {
		...
		$list->addItem(new bzhyCSpan(_('Upper level maps').':'));
 ...
 */
function bzhymakeFormFooter(CButtonInterface $main_button = null, array $other_buttons = []) {
 ...
 * @return CDiv
 */
function bzhygetHostAvailabilityTable($host) {
	...
	foreach (['zbx' => '', 'snmp' => 'snmp_', 'jmx' => 'jmx_', 'ipmi' => 'ipmi_'] as $type => $prefix) {
		switch ($host[$prefix.'available']) {
			case HOST_AVAILABLE_TRUE:
				$ai = (new bzhyCSpan($type))->addClass(ZBX_STYLE_STATUS_GREEN);
				break;
			case HOST_AVAILABLE_FALSE:
				$ai = (new bzhyCSpan($type))
					->addClass(ZBX_STYLE_STATUS_RED)
					->addClass(ZBX_STYLE_CURSOR_POINTER)
					->setHint($host[$prefix.'error'], ZBX_STYLE_RED);
				break;
			case HOST_AVAILABLE_UNKNOWN:
				$ai = (new bzhyCSpan($type))->addClass(ZBX_STYLE_STATUS_GREY);
				break;
		}
 ...
 * @return CDiv
 */
function bzhygetHostGroupLifetimeIndicator($current_time, $ts_delete) {
 ...
 * @return CDiv
 */
function bzhygetHostLifetimeIndicator($current_time, $ts_delete) {
	// Check if the element should've been deleted in the past.
	if ($current_time > $ts_delete) {
		$warning = _(
 ...
 * @return CDiv
 */
function bzhygetApplicationLifetimeIndicator($current_time, $ts_delete) {
	// Check if the element should've been deleted in the past.
	if ($current_time > $ts_delete) {
		$warning = _(
 ...
 * @return CDiv
 */
function bzhygetItemLifetimeIndicator($current_time, $ts_delete) {
	// Check if the element should've been deleted in the past.
	if ($current_time > $ts_delete) {
		$warning = _(
 ...
 * @return array
 */
function bzhycreateDateSelector($name, $date, $relatedCalendar = null) {
 ...
 * @return CDiv
 */
function bzhymakePageFooter($with_version = true)
{
	return (new CDiv([
 ...
 * @return CComboBox
 */
function bzhymakeAdministrationGeneralMenu($selected)
{
 ...
 * @return bzhyCSpan
 */
function bzhymakeInformationList($info_icons)
{
 ...
 * @return bzhyCSpan
 */
function bzhymakeInformationIcon($message)
{
	return (new bzhyCSpan())
 ...
 * @return bzhyCSpan
 */
function bzhymakeErrorIcon($error)
{
	return (new bzhyCSpan())
		->addClass(ZBX_STYLE_ICON_INFO)
		->addClass(ZBX_STYLE_STATUS_RED)
 ...
 * @return bzhyCSpan
 */
function bzhymakeUnknownIcon($error)
{
	return (new bzhyCSpan())
		->addClass(ZBX_STYLE_ICON_INFO)
 ...
 * @return bzhyCSpan
 */
function bzhymakeWarningIcon($error)
{
	return (new bzhyCSpan())
		->addClass(ZBX_STYLE_ICON_INFO)
 ...
 * @return string
 */
function bzhygetTriggerSeverityCss($config)
{
	$severities = [

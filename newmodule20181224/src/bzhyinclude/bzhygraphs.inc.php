...
function bzhygraphType($type = null) {
	$types = [
		BZHYGRAPH_TYPE_NORMAL => _('Normal'),
		BZHYGRAPH_TYPE_STACKED => _('Stacked'),
		BZHYGRAPH_TYPE_PIE => _('Pie'),
		BZHYGRAPH_TYPE_EXPLODED => _('Exploded')
	];
...
function bzhygraph_item_drawtypes() {
	return [
		BZHYGRAPH_ITEM_DRAWTYPE_LINE,
		BZHYGRAPH_ITEM_DRAWTYPE_FILLED_REGION,
		BZHYGRAPH_ITEM_DRAWTYPE_BOLD_LINE,
		BZHYGRAPH_ITEM_DRAWTYPE_DOT,
		BZHYGRAPH_ITEM_DRAWTYPE_DASHED_LINE,
		BZHYGRAPH_ITEM_DRAWTYPE_GRADIENT_LINE
	];
}

function bzhygraph_item_drawtype2str($drawtype) {
	switch ($drawtype) {
		case BZHYGRAPH_ITEM_DRAWTYPE_LINE:
			return _('Line');
		case BZHYGRAPH_ITEM_DRAWTYPE_FILLED_REGION:
			return _('Filled region');
		case BZHYGRAPH_ITEM_DRAWTYPE_BOLD_LINE:
			return _('Bold line');
		case BZHYGRAPH_ITEM_DRAWTYPE_DOT:
			return _('Dot');
		case BZHYGRAPH_ITEM_DRAWTYPE_DASHED_LINE:
			return _('Dashed line');
		case BZHYGRAPH_ITEM_DRAWTYPE_GRADIENT_LINE:
...
function bzhygraph_item_calc_fnc2str($calc_fnc) {
	switch ($calc_fnc) {
		case 0:
			return _('Count');
		case BZHYCALC_FNC_ALL:
			return _('all');
		case BZHYCALC_FNC_MIN:
			return _('min');
		case BZHYCALC_FNC_MAX:
			return _('max');
		case BZHYCALC_FNC_LST:
			return _('last');
		case BZHYCALC_FNC_AVG:
		default:
			return _('avg');
	}
}

function bzhygetGraphDims($graphid = null) {
	$graphDims = [];
	...
	if (is_null($graphid)) {
		...
		if (BZHYGRAPH_YAXIS_SIDE_DEFAULT == 0) {
	...
	$dbGraphs = bzhyDBselect(
		...
		' WHERE g.graphid='.bzhy_dbstr($graphid).
			' AND gi.graphid=g.graphid'
	);
	if ($graph = bzhyDBfetch($dbGraphs)) {
...
function bzhyget_realhosts_by_graphid($graphid) {
	$graph = bzhygetGraphByGraphId($graphid);
	if (!empty($graph['templateid'])) {
		return bzhyget_realhosts_by_graphid($graph['templateid']);
	}
	return bzhyget_hosts_by_graphid($graphid);
}

function bzhyget_hosts_by_graphid($graphid) {
	return bzhyDBselect(
		'SELECT DISTINCT h.*'.
		' FROM graphs_items gi,items i,hosts h'.
		' WHERE h.hostid=i.hostid'.
			' AND gi.itemid=i.itemid'.
			' AND gi.graphid='.bzhy_dbstr($graphid)
	);
}
 ...
 */
function bzhyget_min_itemclock_by_graphid($graphid) {
	$items = bzhyDBfetchArray(DBselect(
 ...
 * @return int (unixtime)
 */
function bzhyget_min_itemclock_by_itemid($items) {
	$item_types = [
		BZHYITEM_VALUE_TYPE_FLOAT => [],
		BZHYITEM_VALUE_TYPE_STR => [],
		BZHYITEM_VALUE_TYPE_LOG => [],
		BZHYITEM_VALUE_TYPE_UINT64 => [],
		BZHYITEM_VALUE_TYPE_TEXT => []
	];
	...
	foreach ($items as $item) {
		...
		if ($item['value_type'] == BZHYITEM_VALUE_TYPE_FLOAT || $item['value_type'] == BZHYITEM_VALUE_TYPE_UINT64) {
	...
	foreach ($item_types as $type => $itemids) {
		if ($itemids) {
			switch ($type) {
				case BZHYITEM_VALUE_TYPE_FLOAT:
					$sql_from = ($max['history'] > $max['trends']) ? 'history' : 'trends';
					break;
				case BZHYITEM_VALUE_TYPE_STR:
					$sql_from = 'history_str';
					break;
				case BZHYITEM_VALUE_TYPE_LOG:
					$sql_from = 'history_log';
					break;
				case BZHYITEM_VALUE_TYPE_UINT64:
					$sql_from = ($max['history'] > $max['trends']) ? 'history_uint' : 'trends_uint';
					break;
				case BZHYITEM_VALUE_TYPE_TEXT:
			...
			foreach ($itemids as $itemid) {
				$sql_unions[] =
					...
					' FROM '.$sql_from.' h'.
					' WHERE h.itemid='.bzhy_dbstr($itemid);
			}
		}
	}

	$row = bzhyDBfetch(bzhyDBselect(
	...
	if ($min_clock == 0) {
		if ($item_types[BZHYITEM_VALUE_TYPE_FLOAT] || $item_types[BZHYITEM_VALUE_TYPE_UINT64]) {
			$min_clock = time() - BZHYSEC_PER_DAY * max($max['history'], $max['trends']);
			...
			$min_clock = time() - BZHYSEC_PER_YEAR;
		}
	}
...
function bzhygetGraphByGraphId($graphId) {
	$dbGraph = bzhyDBfetch(bzhyDBselect('SELECT g.* FROM graphs g WHERE g.graphid='.bzhy_dbstr($graphId)));

	if ($dbGraph) {
		return $dbGraph;
	}

	bzhyerror(_s('No graph item with graphid "%s".', $graphId));
 ...
 * @return array|bool
 */
function bzhygetSameGraphItemsForHost($gitems, $destinationHostId, $error = true, array $flags = []) {
	$result = [];

	$flagsSql = $flags ? ' AND '.bzhydbConditionInt('dest.flags', $flags) : '';
	...
	foreach ($gitems as $gitem) {
		$dbItem = DBfetch(DBselect(
			...
			' WHERE dest.key_=src.key_'.
				' AND dest.hostid='.bzhy_dbstr($destinationHostId).
				' AND src.itemid='.bzhy_dbstr($gitem['itemid']).
				$flagsSql
		));
		...
		elseif ($error) {
			$item = bzhyget_item_by_itemid($gitem['itemid']);
			$host = bzhyget_host_by_hostid($destinationHostId);

			bzhyerror(_s('Missing key "%1$s" for host "%2$s".', $item['key_'], $host['host']));
 ...
 * @return array
 */
function bzhycopyGraphToHost($graphid, $hostid) {
...
function navigation_bar_calc($idx = null, $idx2 = 0, $update = false) {
	...
	elseif ($_REQUEST['period'] > ZBX_MAX_PERIOD) {
		show_error_message(_n('Maximum time period to display is %1$s day.',
			...
			(int) ZBX_MAX_PERIOD / BZHYSEC_PER_DAY
		));

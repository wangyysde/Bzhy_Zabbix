...
if [ -f "${moduledir}/install_scipts/${OVERRRIDELST}" ]; then
    ...
    for file in `cat ${overridelst}`;do
        ...
        if [ "X${pathname}" == "X" ]; then
            if [ ! -f "${zabbixdir}/org_bzhy_${filename}" ]; then
                /bin/cp -Rpf "${zabbixdir}${destfile}"  "${zabbixdir}/org_bzhy_${filename}"
            fi
        else
            if [ ! -f "${zabbixdir}/${pathname}/org_bzhy_${filename}" ]; then
                /bin/cp -Rpf "${zabbixdir}${destfile}"  "${zabbixdir}/${pathname}org_bzhy_${filename}"
            fi
        fi
